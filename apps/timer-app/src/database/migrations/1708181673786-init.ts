...
export class Init1708181673786 implements MigrationInterface {
  name = 'Init1708181673786';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`study_record\` (\`study_record_id\` bigint NOT NULL AUTO_INCREMENT, \`status\` varchar(20) NULL, \`start_time\` datetime NOT NULL, \`end_time\` datetime NULL, \`deleted_at\` datetime(6) NULL, \`member_id\` varchar(36) NOT NULL, \`study_category_id\` bigint NULL, PRIMARY KEY (\`study_record_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`study_category\` (\`study_category_id\` bigint NOT NULL AUTO_INCREMENT, \`subject\` varchar(50) NOT NULL, \`color\` varchar(7) NOT NULL DEFAULT '#000000', \`deleted_at\` datetime(6) NULL, \`member_id\` varchar(36) NOT NULL, PRIMARY KEY (\`study_category_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`account\` (\`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`account_id\` varchar(36) NOT NULL, \`email\` varchar(255) NULL, \`password\` varchar(255) NULL, \`provider\` varchar(255) NOT NULL DEFAULT 'email', \`social_id\` varchar(255) NULL, \`roleId\` int NULL, INDEX \`IDX_d88d87a719d5c228e680e4daa0\` (\`social_id\`), UNIQUE INDEX \`IDX_4c8f96ccf523e9a3faefd5bdd4\` (\`email\`), PRIMARY KEY (\`account_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`member\` (\`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`member_id\` varchar(36) NOT NULL, \`nickname\` varchar(100) NOT NULL, \`email\` varchar(100) NOT NULL, \`image_url\` varchar(255) NULL, \`active_record_id\` bigint NULL, \`point\` int NOT NULL DEFAULT '0', \`study_streak_id\` int NULL, \`account_id\` varchar(36) NULL, UNIQUE INDEX \`REL_51b8fd29e951724b448d063522\` (\`study_streak_id\`), UNIQUE INDEX \`REL_a706d6db681a07b5f485eff318\` (\`account_id\`), PRIMARY KEY (\`member_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`study_record\` ADD CONSTRAINT \`FK_9597be6d50b876ab9418893ed86\` FOREIGN KEY (\`study_category_id\`) REFERENCES \`study_category\`(\`study_category_id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`study_category\` ADD CONSTRAINT \`FK_f85be32327b5dee5d2f990739a0\` FOREIGN KEY (\`member_id\`) REFERENCES \`member\`(\`member_id\`) ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`study_category\` DROP FOREIGN KEY \`FK_f85be32327b5dee5d2f990739a0\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`study_record\` DROP FOREIGN KEY \`FK_9597be6d50b876ab9418893ed86\``,
    );
    await queryRunner.query(
